...
func (s *snapshotter) createSnapshot(ctx context.Context, kind snapshots.Kind, key, parent string, opts []snapshots.Opt) ([]mount.Mount, error) {
	...
	if kind == snapshots.KindActive {
		...
		var sizeGB uint
		if sizeGBstr, ok := snapshotInfo.Labels["io.microsoft.container.storage.rootfs.size-gb"]; ok {
			i64, _ := strconv.ParseUint(sizeGBstr, 10, 64)
			sizeGB = uint(i64)
		}

		var scratchLocation string
		scratchLocation, _ = snapshotInfo.Labels["io.microsoft.container.storage.rootfs.location"]
...
func (s *snapshotter) openOrCreateScratch(ctx context.Context, sizeGB uint, scratchLoc string) (_ *os.File, err error) {
	...
	if err != nil {
		...
		if err := rhcs.CreateScratch(ctx, scratchTempPath, sizeGB); err != nil {
